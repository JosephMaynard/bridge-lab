...
export const presets: PresetScenario[] = [
  ...
  },
  {
    ...
    config: {
      ...
      dinosaur: { ...defaultConfig.dinosaur, enabled: false },
  ...
  },
  {
    id: "dinosaur",
    name: "T-Rex Attack",
    description: "A giant low-poly predator bites the side span with pulsed side loading.",
    config: {
      ...cloneConfig(defaultConfig),
      timing: { ...defaultConfig.timing, duration: 22 },
      bridge: { ...defaultConfig.bridge, stiffness: 1.08, damping: 0.48, failureThreshold: 1.1, materialStrength: 0.96 },
      wind: { ...defaultConfig.wind, speed: 22, gustiness: 0.24, turbulence: 0.32, particleCount: 700 },
      load: { ...defaultConfig.load, totalWeight: 54, distribution: "offset", bias: -0.2, movingLoad: true, movingSpeed: 0.26 },
      dinosaur: { ...defaultConfig.dinosaur, enabled: true, attackTime: 2.4, intensity: 1.05, targetBias: -0.24, side: "near", scale: 5.7, biteFrequency: 1.05 },
      environment: { timeOfDay: "golden" },
    },
  },
  {
    ...
    config: {
      ...
      dinosaur: { ...defaultConfig.dinosaur, enabled: true, attackTime: 3, intensity: 0.82, targetBias: 0.22, side: "far", scale: 5.3, biteFrequency: 1.2 },
